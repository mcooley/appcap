...
internal static class E2EHelpers
{
    ...
    {
        if (!IsEnabled(Environment.GetEnvironmentVariable("RUNMC_E2E")))
        {
            throw new InvalidOperationException("Set RUNMC_E2E=1 to run end-to-end tests.");
        }

        string? executablePath = Environment.GetEnvironmentVariable("RUNMC_E2E_EXECUTABLE");
        if (string.IsNullOrWhiteSpace(executablePath))
        {
            throw new InvalidOperationException("Set RUNMC_E2E_EXECUTABLE to a previously-built RunMc.exe path.");
        }

        string fullExecutablePath = Path.GetFullPath(executablePath);
        if (!File.Exists(fullExecutablePath))
        {
            throw new InvalidOperationException($"RUNMC_E2E_EXECUTABLE does not exist: {fullExecutablePath}");
        }

        string outputDirectory = Path.Combine(Path.GetTempPath(), "runmc-e2e", Guid.NewGuid().ToString("N"));
    ...
    {
        foreach (Process process in Process.GetProcessesByName("RunMc.TestApp"))
        {
            using (process)
            {
